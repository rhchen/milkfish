...
public class ControlFlowView extends AbstractTimeGraphView {
     ...
     * View ID.
     */
    public static final String ID = "net.sf.milkfish.systrace.android.ui.views.controlflow.ControlFlowView"; //$NON-NLS-1$
    ...
    protected void buildEventList(final ITmfTrace trace, IProgressMonitor monitor) {
        ...
        for (ITmfTrace aTrace : TmfTraceManager.getTraceSet(trace)) {
            ...
            if (aTrace instanceof AndroidTrace) {
                ...
                AndroidTrace ctfKernelTrace = (AndroidTrace) aTrace;
                ITmfStateSystem ssq = ctfKernelTrace.getStateSystems().get(AndroidTrace.STATE_ID);
                ...
                List<Integer> threadQuarks = ssq.getQuarks(SystraceAttributes.THREADS, "*"); //$NON-NLS-1$
                for (int threadQuark : threadQuarks) {
                    ...
                    try {
                        try {
                            execNameQuark = ssq.getQuarkRelative(threadQuark, SystraceAttributes.EXEC_NAME);
                        } catch (AttributeNotFoundException e) {
                            continue;
                        }
                        int ppidQuark = ssq.getQuarkRelative(threadQuark, SystraceAttributes.PPID);
    ...
    protected List<ITimeEvent> getEventList(TimeGraphEntry tgentry, long startTime, long endTime, long resolution, IProgressMonitor monitor) {
        ...
        ITmfStateSystem ssq = entry.getTrace().getStateSystems().get(AndroidTrace.STATE_ID);
        try {
            int statusQuark = ssq.getQuarkRelative(entry.getThreadQuark(), SystraceAttributes.STATUS);
    ...
    private int getSelectionValue(long time) {
        ...
        for (ITmfTrace trace : fTraceManager.getActiveTraceSet()) {
            ...
            if (trace instanceof AndroidTrace) {
            	AndroidTrace ctfKernelTrace = (AndroidTrace) trace;
                ITmfStateSystem ssq = ctfKernelTrace.getStateSystems().get(AndroidTrace.STATE_ID);
                if (time >= ssq.getStartTime() && time <= ssq.getCurrentEndTime()) {
                    List<Integer> currentThreadQuarks = ssq.getQuarks(SystraceAttributes.CPUS, "*", SystraceAttributes.CURRENT_THREAD); //$NON-NLS-1$
                    for (int currentThreadQuark : currentThreadQuarks) {
                        try {
                            ...
                            if (currentThread > 0) {
                                int statusQuark = ssq.getQuarkAbsolute(SystraceAttributes.THREADS, Integer.toString(currentThread), SystraceAttributes.STATUS);
    ...
    protected List<ILinkEvent> getLinkList(long startTime, long endTime, long resolution, IProgressMonitor monitor) {
        ...
        for (ITmfTrace trace : traces) {
            if (trace instanceof AndroidTrace) {
                ITmfStateSystem ssq = trace.getStateSystems().get(AndroidTrace.STATE_ID);
                try {
