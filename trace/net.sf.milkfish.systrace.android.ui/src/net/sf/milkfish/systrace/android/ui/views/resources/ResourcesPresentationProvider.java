...
public class ResourcesPresentationProvider extends TimeGraphPresentationProvider {
    ...
    private static State getEventState(TimeEvent event) {
        if (event.hasValue()) {
            ...
            if (entry.getType() == Type.CPU) {
                if (value == SystraceStateValues.CPU_STATUS_IDLE) {
                    return State.IDLE;
                } else if (value == SystraceStateValues.CPU_STATUS_RUN_USERMODE) {
                    return State.USERMODE;
                } else if (value == SystraceStateValues.CPU_STATUS_RUN_SYSCALL) {
                    return State.SYSCALL;
                } else if (value == SystraceStateValues.CPU_STATUS_IRQ) {
                    return State.IRQ;
                } else if (value == SystraceStateValues.CPU_STATUS_SOFTIRQ) {
                    return State.SOFT_IRQ;
                }
            } else if (entry.getType() == Type.IRQ) {
                return State.IRQ_ACTIVE;
            } else if (entry.getType() == Type.SOFT_IRQ) {
                if (value == SystraceStateValues.SOFT_IRQ_RAISED) {
        ...
        if (event instanceof TimeEvent && ((TimeEvent) event).hasValue()) {
            ...
            if (tcEvent.hasValue()) {
                ...
                // Check for type CPU
                    ...
                    if (status == SystraceStateValues.CPU_STATUS_IRQ) {
                        ...
                        try {
                            ...
                            List<Integer> irqQuarks = ss.getQuarks(SystraceAttributes.RESOURCES, SystraceAttributes.IRQS, "*"); //$NON-NLS-1$
                    ...
                    } else if (status == SystraceStateValues.CPU_STATUS_SOFTIRQ) {
                        ...
                        try {
                            List<ITmfStateInterval> fullState = ss.queryFullState(event.getTime());
                            List<Integer> softIrqQuarks = ss.getQuarks(SystraceAttributes.RESOURCES, SystraceAttributes.SOFT_IRQS, "*"); //$NON-NLS-1$
                    ...
                    } else if (status == SystraceStateValues.CPU_STATUS_RUN_USERMODE || status == SystraceStateValues.CPU_STATUS_RUN_SYSCALL) {
                        ...
                        try {
                            ...
                            if (!interval.getStateValue().isNull()) {
                                ...
                                int execNameQuark = ssq.getQuarkAbsolute(SystraceAttributes.THREADS, Integer.toString(currentThreadId), SystraceAttributes.EXEC_NAME);
                                ...
                                if (status == SystraceStateValues.CPU_STATUS_RUN_SYSCALL) {
                                    int syscallQuark = ssq.getQuarkAbsolute(SystraceAttributes.THREADS, Integer.toString(currentThreadId), SystraceAttributes.SYSTEM_CALL);
    ...
    public void postDrawEvent(ITimeEvent event, Rectangle bounds, GC gc) {
        ...
        int status = tcEvent.getValue();
        if (status != SystraceStateValues.CPU_STATUS_RUN_USERMODE && status != SystraceStateValues.CPU_STATUS_RUN_SYSCALL) {
            return;
        }
        ...
        try {
            while (time < event.getTime() + event.getDuration()) {
                int cpuQuark = entry.getQuark();
                int currentThreadQuark = ss.getQuarkRelative(cpuQuark, SystraceAttributes.CURRENT_THREAD);
                ...
                if (!tidInterval.getStateValue().isNull()) {
                    ...
                    int currentThreadId = value.unboxInt();
                    if (status == SystraceStateValues.CPU_STATUS_RUN_USERMODE && currentThreadId != fLastThreadId) {
                        int execNameQuark = ss.getQuarkAbsolute(SystraceAttributes.THREADS, Integer.toString(currentThreadId), SystraceAttributes.EXEC_NAME);
                    ...
                    } else if (status == SystraceStateValues.CPU_STATUS_RUN_SYSCALL) {
                        int syscallQuark = ss.getQuarkAbsolute(SystraceAttributes.THREADS, Integer.toString(currentThreadId), SystraceAttributes.SYSTEM_CALL);
